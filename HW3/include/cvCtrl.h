...
namespace {
	...
	//skip=0->skip//skip=1->don'tskip
	//destroy=0->destroy//flag=1->don'tdestroy
	extern void displayImage(const char *windowName, cv::Mat imgToshow, int skip = 0, int flag = 0)
	{
	...
	//0: CV_32FC1
	//1: CV_32FC3
	extern cv::Mat elementMul(cv::Mat mat1, cv::Mat mat2, int matType=0)
	{
	...
	//0: CV_32FC1
	//1: CV_32FC3
	extern cv::Mat elementDiv(cv::Mat mat1, cv::Mat mat2, int matType = 0)
	{
		int matr, matc, mat2r, mat2c;
		matr = mat1.rows; matc = mat1.cols; mat2r = mat2.rows; mat2c = mat2.cols;
		if ((matr != mat2r) || (matc != mat2c))
